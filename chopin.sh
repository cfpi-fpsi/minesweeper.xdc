...
name="Application"
EXCLUDE_PATHS="webxdc.js LICENSE README README.md *.sh *.xdc *~"

function usage() {
    echo "Usage: ${0} [-hfgp] [-e PATH]"
    echo "  -h          Display this message"
    echo "  -e [PATH]   Filename or folder to exclude from package. For several"
    echo "              files or folders, use this flag several times with one argument."
...
function webxdc_create() {
	...
	zip -9 --recurse-paths "${name}.xdc" --exclude $EXCLUDE_PATHS -- * >/dev/null
...
[[ "$#" -eq 0 ]] && usage
while getopts ":h:egpf" option; do
    case ${option} in
	h)
	    usage
	    ;;
